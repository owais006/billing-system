//Developer: Muhhammad Owais Quadri
#include<iostream>
#include<process.h>
#include<fstream>
#include <stdlib.h>
...
void head::output()
{
	...
	outfile<<"------------------------------------------------------------------------"<<endl;
cout<<"\n";
	cout<<"Name of Item\tHSN ACS\t\tQuantity   Price  Total Price\n";
	for(int i=0;i<totalitems;i++)
	{
        // outfile<<"Name: "<<"\t"<<"HSN ACS: "<<"\t"<<" Qty: "<<"\t"<<" Rate: "<<endl;
		outfile<<"Name: "<<Iname[i]<<"\t"<<"HSN ACS: "<<hsn[i]<<" Qty: "<<Qty[i]<<" Rate: "<<tprice[i]<<endl;
        // outfile<<Iname[i]<<"\t"<<hsn[i]<<"\t"<<Qty[i]<<"\t"<<tprice[i]<<endl;
		cout<<Iname[i]<<"\t\t"<<hsn[i]<<"\t\t   "<<Qty[i]<<"\t     "<<price[i]<<"\t     "<<tprice[i]<<'\n';
	}
...
void gst::vatcal()
{
	...
	for(int i=0;i<totalitems;i++)
	{
		if(price[i]<=100.00)
		{
			vatprice[i]=tprice[i]+(0.18*tprice[i]);
		}
		else
		{
			vatprice[i]=tprice[i]+(0.18*tprice[i]);
...
start:
	...
	cout<<"\n\n\t\t---------------------------------------------";
	cout<<"\n\t\t\t\tWINCRAFT (INDIA)";
    cout<<"\n\t\tC-184/17A street no.3 Chauhan Bangar Delhi-110053";
    cout<<"\n\t\t\tE-mail: agecoinda@gamil.com";
	cout<<"\n\t\t---------------------------------------------";
	 cout<<"\n\n\t\t\tWhat you want to do?";
